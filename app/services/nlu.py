...
def _detect_time_pref(t: str) -> str:
    t = t.lower()
    if any(p in t for p in ["más tarde", "mas tarde", "por la tarde", "tarde"]):
        return "tarde"
    if any(p in t for p in ["por la mañana", "mañana temprano", "a primera hora", "temprano"]):
        return "manana"
    if any(p in t for p in ["noche", "tarde-noche", "nochecita"]):
        return "noche"
    return ""

def _is_farewell(t: str) -> bool:
    t = t.strip().lower()
    return t in {
...
def _keyword_router(texto: str) -> dict:
    ...
    # Hora explícita → tratamos como booking
    if re.search(r"\b([01]?\d|2[0-3]):([0-5]\d)\b", t) or re.search(r"\b([1-9]|1[0-2])\s*(am|pm)\b", t):
        tp = _detect_time_pref(t)
        return {"intent":"book","entities":{"time_pref": tp},"reply":"Entendido, ¿para qué **día** es esa hora?"}

    # Saludo inicial con menú formateado
    ...
    # Preferencias de turno o lenguaje de agenda
    if any(k in t for k in ["agendar","cita","sacar cita","reservar","programar",
                            "más tarde","mas tarde","por la tarde","tarde",
                            "por la mañana","temprano","noche","tarde-noche"]):
        tp = _detect_time_pref(t)
        return {"intent":"book","entities":{"time_pref": tp},"reply":"📅 ¡Perfecto! ¿Qué **día** te gustaría?"}

    if any(k in t for k in ["cambiar","reagendar","modificar","mover","reprogramar"]):
        tp = _detect_time_pref(t)
        return {"intent":"reschedule","entities":{"time_pref": tp},"reply":"Claro, ¿qué **día** te conviene?"}
