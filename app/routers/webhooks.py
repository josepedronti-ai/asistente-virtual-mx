...
async def whatsapp_webhook(From: str = Form(None), Body: str = Form(None)) -> str:
    ...
    # --- Intención NLU (info / cancelar / reservar etc.) ---
    ...
    if intent == "info":
        if entities.get("topic") in ("costos","costo","precio","precios"):
            send_text(From, polish(generate_reply("prices", {})))
            return ""
        if entities.get("topic") in ("ubicacion","ubicación","direccion","dirección"):
    ...
    if state == "idle":
        ...
        # Si no entendimos, fallback educado
    ...
    if state == "await_time":
        ...
        if not time_hint:
            # si el usuario pide lista otra vez, reenviamos
        ...
        if not parsed_date:
            # por seguridad pedimos fecha de nuevo
            send_text(From, polish(generate_reply("ask_date_strict", {})))
            _set_state(From, "await_date")
            return ""
        for db in db_session():
    ...
    if state == "await_confirm":
        if is_yes(raw_text):
            # Confirmó → ahora sí pedimos NOMBRE (no agendamos aún)
            send_text(From, polish(generate_reply("need_name", {})))
            _set_state(From, "await_name")
            return ""
        if is_no(raw_text):
            # No confirmó → volver a pedir fecha
            send_text(From, polish(generate_reply("ask_date_strict", {})))
            _set_state(From, "await_date")
            return ""
        # Si no entendemos, volvemos a preguntar confirmación
    ...
    if state == "await_name":
        ...
        if not last_date or not last_time:
            # algo se perdió; reiniciar flujo
            send_text(From, polish(generate_reply("ask_date_strict", {})))
            _set_state(From, "await_date")
            return ""
        h, m = last_time
        ...
        for db in db_session():
            ...
            # crea/mueve cita (RESERVED) y luego CONFIRM + CALENDAR
            appt = move_or_create_appointment(db, patient, start_dt)

            # Confirmar cita + calendar
            ...
            if not appt.event_id:
                try:
                    ...
                    # Si falla calendar, igual dejamos confirmada en DB
                    pass
            db.commit()
